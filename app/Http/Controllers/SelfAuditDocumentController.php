...
class SelfAuditDocumentController extends Controller
{
    ...
    public function __construct(
        ...
        $this->middleware('permission:menu_store.cnr.self_audit_documents.index|menu_store.cnr.self_audit_documents.create|menu_store.cnr.self_audit_documents.update|menu_store.cnr.self_audit_documents.delete');
    }
    ...
    public function index($id, $year, $month_number)
    {
        try {
            ...
            $permissions = [
                'create' => ['menu_store.cnr.self_audit_documents.create'],
                'update' => ['menu_store.cnr.self_audit_documents.update'],
                'delete' => ['menu_store.cnr.self_audit_documents.delete'],
            ];

            $folders = Tag::whereIn('id', [1,2,3])->get();
            ...
            ->whereIn('store_document_tags.tag_id', [1,2,3]);
            ...
            $breadCrumb = ['Compliance & Regulatory', 'Self-Audit Documents for Year '.$year];
            return view('/stores/compliance/selfAuditDocuments/index', compact('breadCrumb', 'folders', 'permissions', 'icons', 'monthlyCountFolders', 'monthlyAllCountFolders'));
        } catch (\Throwable $th) {
        ...
        if($request->ajax()){
            ...
            try {
                ...
                return response()->json([
                    'error' => $e->getMessage(),
                    'message' => 'Something went wrong in SelfAuditDocumentController.store.db_transaction.'
                ]);
            }
        }
    }
