...
class ForDeliveryTodayController extends Controller
{
    ...
    public function __construct(UploadInterface $repository)
    {
        ...
        $this->middleware('permission:menu_store.operations.for_delivery_today.index');
    }
    ...
    public function index($id)
    {
        try {
            ...
            $breadCrumb = ['Operations', 'For Delivery Today'];
            return view('/stores/operations/forDeliveryToday/index', compact('breadCrumb'));
        } catch (\Throwable $th) {
    ...
    public function data(Request $request)
    {
        if($request->ajax()){
            ...
            $query = OperationOrder::with('file')->where('status', 'For Delivery Today');
            ...
            if($request->has('delivered_date')) {
                ...
                if(!empty($delivered_date)) {
                    ...
                    $query = $query->where('delivered_date', $delivered_date);
                }
            }
            ...
            $hideD = 'hidden';
            $hideAll = 'hidden';
            if(auth()->user()->can('menu_store.operations.for_delivery_today.update')) {
                $hideU = ''; $hideAll = '';
            }
            if(auth()->user()->can('menu_store.operations.for_delivery_today.delete')) {
                $hideD = ''; $hideAll = '';
            }
            ...
            foreach ($data as $value) {
                ...
                $newData[] = [
                    ...
                    'actions' =>  '<div class="d-flex order-actions" '.$hideAll.'>
                        ...
                        id="for-delivery-today-edit-btn-'.$value->id.'"
    ...
    public function upload(Request $request)
    {
        try {
            if($request->ajax()){
                DB::beginTransaction();
                $this->repository->uploadOperationOrderFDT($request);
                DB::commit();
            ...
            if($request->ajax()){
                ...
                foreach ($checkedIds as $id) {
                    ...
                    if ($order && $order->is_completed == 0) {
                        ...
                        $order->update(['is_completed' => 1, 'delivered_date' => $today]);
                    }
                }
                ...
                foreach ($uncheckedIds as $id) {
                    ...
                    if ($order && $order->is_completed == 1) {
                        $order->update(['is_completed' => 0, 'delivered_date' => null]);
                    }
                }
    ...
    public function uploadShippingLabel(Request $request, $id = null)
    {
        try {
            if($request->ajax()){
                DB::beginTransaction();
                $request->merge(['id' => $id]);
                $status_type = 'For Delivery Today';
