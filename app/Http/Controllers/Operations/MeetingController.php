...
class MeetingController extends Controller
{
    ...
    public function __construct(
        ...
        $this->middleware('permission:menu_store.operations.meetings.index|menu_store.operations.meetings.create|menu_store.operations.meetings.update|menu_store.operations.meetings.delete');
    }
    ...
    public function index($id, $year, $month_number)
    {
        try {
            ...
            $page_id = 79;
            $permissions = [
                'create' => ['menu_store.operations.meetings.create'],
                'update' => ['menu_store.operations.meetings.update'],
                'delete' => ['menu_store.operations.meetings.delete'],
            ];
            $folders = StoreFolder::with('files.tag')
                ->where('page_id', 79)->get();
            ...
            $breadCrumb = ['Operation', 'Meetings'];
            return view('/stores/operations/meetings/index', compact('breadCrumb', 'folders', 'page_id', 'permissions', 'icons', 'monthlyCountFolders', 'weeks'));
        } catch (\Throwable $th) {
