...
use App\Exports\ClinicalRxDailyCensusCustomExport;
use App\Http\Controllers\Controller;
use App\Imports\ClinicalRxDailyCensusImport;
use App\Jobs\NotifyClinicalRxDailyCensusToCustom;
use App\Mail\ClinicalRxDailyCensusMail;
use App\Mail\ResponsiveTableMail;
...
class RxDailyCensusController extends Controller
{
    public function __construct()
    {

        $this->middleware('permission:menu_store.clinical.rx_daily_census.index|menu_store.clinical.rx_daily_census.create|menu_store.clinical.rx_daily_census.update|menu_store.clinical.rx_daily_census.delete');
    }
    ...
    public function index($id)
    {
        try {
            ...
            $breadCrumb = ['Clinical', 'TRHC px+Rx census'];
            return view('/stores/clinical/rxDailyCensus/index', compact('breadCrumb'));
        } catch (\Throwable $th) {
    ...
    public function import($id, Request $request)
    {
        try {
            ...
            Excel::import(new ClinicalRxDailyCensusImport($params), $request->file('upload_file'));
            ...
            return response()->json([
                'error' => $e->getMessage(),
                'message' => 'Something went wrong in RxDailyCensusController.import.'
            ]);
        }
    }
    ...
    public function dateWith($id, Request $request)
    {
        try {
            if($request->ajax()){
                ...
                $clinicalKpiDates = ClinicalRxDailyCensus::with('patient')
            ...
            return response()->json([
                'error' => $e->getMessage(),
                'message' => 'Something went wrong in RxDailyCensusController.date_with.'
            ]);
        }
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            return response()->json([
                'error' => $e->getMessage(),
                'message' => 'Something went wrong in RxDailyCensusController.store.'
            ]);
        }
    }
    ...
    public function delete(Request $request)
    {
        try {
            ...
            if($request->ajax()){
                ...
                $co = ClinicalRxDailyCensus::findOrFail($request->id);
                $co->delete();
            ...
            return response()->json([
                'error' => $e->getMessage(),
                'message' => 'Something went wrong in RxDailyCensusController.delete.'
            ]);
        }
    }
        ...
        try {
            ...
            if($request->ajax()){
                ...
                if(count($selectedIds) > 0) {
                    $count = ClinicalRxDailyCensus::whereIn('id', $selectedIds)->delete();
                    if(!$count) {
            ...
            return response()->json([
                'error' => $e->getMessage(),
                'message' => 'Something went wrong in RxDailyCensusController.deleteAll'
            ]);
        }
    }
    ...
    public function update(Request $request)
    {
        try {
            ...
            if($request->ajax()){
                ...
                $co = ClinicalRxDailyCensus::findOrFail($data['id']);
                $co->date = $data['date'];
                $co->pharmacy_store_id = $data['pharmacy_store_id'];
                $co->scripts_received = $data['scripts_received'];
            ...
            return response()->json([
                'error' => $e->getMessage(),
                'message' => 'Something went wrong in RxDailyCensusController.update.'
            ]);
        }
    }
    ...
    public function data(Request $request)
    {
        try {
            if($request->ajax()){
                ...
                $query = ClinicalRxDailyCensus::with('user.employee', 'patient')
                ...
                foreach ($data as $value) {
                    ...
                    if(auth()->user()->can('menu_store.clinical.rx_daily_census.update')) {
                        $actions .= '
                    ...
                    if(auth()->user()->can('menu_store.clinical.rx_daily_census.delete')) {
                    ...
                    $newData[] = [
                        ...
                        'birth_date' => !empty($value->birth_date) ? date('Y-m-d', strtotime($value->birth_date)) : '',
                        'formatted_birth_date' => !empty($value->birth_date) ? date('m/d/Y', strtotime($value->birth_date)) : '',
                        'scripts_received' => $value->scripts_received,
            ...
            return response()->json([
                'error' => $e->getMessage(),
                'message' => 'Something went wrong in RxDailyCensusController.data.'
            ]);
        }
    }
    ...
    public function summary(Request $request)
    {
        try {
            ...
            $uniquesObject = ClinicalRxDailyCensus::select(
            ...
            $scriptsObject = ClinicalRxDailyCensus::select(
                    DB::raw('COUNT(DISTINCT patient_name) as count_patient_with_scripts')
                )
                ->where('date', $date)
                ->where('scripts_received', '>', 0)
                ->where('pharmacy_store_id', $pharmacy_store_id)
                ->first();

            $topObject = ClinicalRxDailyCensus::select(
                    DB::raw('SUM(scripts_received) as sum_scripts_received'),
                    'provider'
                )
                ->where('date', $date)
                ...
                ->where('pharmacy_store_id', $pharmacy_store_id)
                ->groupBy('provider')
                ->orderBy('sum_scripts_received', 'desc')
                ->limit(3)->get();
            ...
            $data = [
                ...
                'summary_unique_patient_with_scripts' => isset($scriptsObject->count_patient_with_scripts) ? $scriptsObject->count_patient_with_scripts : 0,
                'summary_top_providers' => $topObject,
            ];
            ...
            return response()->json([
                'error' => $e->getMessage(),
                'message' => 'Something went wrong in RxDailyCensusController.summary.'
            ]);
        }
    }
    ...
    public function sendMail(Request $request)
    {
        try {
            if($request->ajax()){
                ...
                //     Bus::dispatch(new NotifyClinicalRxDailyCensusToCustom($query, $summary, $emails, $user));
                // }
                ...
                if(config('mail.maintenance') != "ON") {
                    Mail::to($userEmail)
                        ->cc($cc)
                        ->send(new ClinicalRxDailyCensusMail($data));
                }
            ...
            return response()->json([
                'error' => $e->getMessage(),
                'message' => 'Something went wrong in RxDailyCensusController.sendMail.'
            ]);
        }
    }
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new ClinicalRxDailyCensusCustomExport($filterArray), 'Clinical Rx Daily Census for Date '.$date.' [Intranet'.$date_today.'].xlsx');
    }
