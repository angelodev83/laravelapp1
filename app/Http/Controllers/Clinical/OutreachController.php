...
use App\Models\ClinicalOutreach;
...
class OutreachController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware('permission:menu_store.clinical.outreach.index|menu_store.clinical.outreach.create|menu_store.clinical.outreach.update|menu_store.clinical.outreach.delete');
    }
    ...
    public function index($id)
    {
        try {
            ...
            $breadCrumb = ['Clinical', 'Outreach'];
            return view('/stores/clinical/outreach/index', compact('breadCrumb'));
        } catch (\Throwable $th) {
    ...
    public function date_with(Request $request)
    {
        if($request->ajax()){
            ...
            $clinicalKpiDates = ClinicalOutreach::with('patient')->whereBetween('date', [$startDate, $endDate])->get();
    ...
    public function store(Request $request)
    {
        try {
            ...
            return response()->json([
                'error' => $e->getMessage(),
                'message' => 'Something went wrong in OutreachController.store.'
            ]);
        }
    }
    ...
    public function delete(Request $request)
    {
        try {
            ...
            if($request->ajax()){
                ...
                $co = ClinicalOutreach::findOrFail($request->id);
        ...
        try {
            ...
            if($request->ajax()){
                ...
                if ($validation->passes()){
                    ...
                    $co = ClinicalOutreach::findOrFail($data['id']);
                    $co->date = $data['date'];
                    $co->patient_id = $data['patient_id'];
                    $co->in_charge = $data['employee_id'];
                    $co->reason = $data['reason'];
            ...
            return response()->json([
                'error' => $e->getMessage(),
                'message' => 'Something went wrong in OutreachController.update.'
            ]);
        }
    }
    ...
    public function data(Request $request)
    {
        if($request->ajax()){
            ...
            $query = ClinicalOutreach::with('user.employee', 'inCharge', 'patient', 'diagnoses.status', 'callStatus', 'providerStatus')
            ...
            foreach ($data as $value) {
                ...
                if(auth()->user()->can('menu_store.clinical.outreach.update')) {
                    $actions .= '
                ...
                if(auth()->user()->can('menu_store.clinical.outreach.delete')) {
    ...
    public function data_schedule(Request $request)
    {
        if($request->ajax()){
            ...
            // get data from products table
            $query = ClinicalOutreach::with('user.employee')
                ->where('pharmacy_store_id', $request->pharmacy_store_id)
