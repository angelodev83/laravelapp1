...
class ClinicalTherapyChangeAndRecoNotification extends Notification
{
    use Queueable;

    private $therapyChangeAndReco, $summary, $emails;

    /**
     * Create a new notification instance.
     */
    public function __construct($therapyChangeAndReco, $summary, $emails)
    {
    ...
    {
        $therapyChangeAndReco = $this->therapyChangeAndReco;
        ...
        if(!empty($therapyChangeAndReco)) {

            $date = $summary->formatted_date ?: '';

            $emailSubject = 'Therapy change+reco for '.$date;
            ...
            <tr>
                ...
                <th style="text-align: left; width: 20%; border: 1px solid #eee; padding: 5px;">Brand Recommended</th>
                <th style="text-align: left; width: 15%; border: 1px solid #eee; padding: 5px;">Status</th>
                <th style="text-align: left; width: 15%; border: 1px solid #eee; padding: 5px;">Provider</th>
                <th style="text-align: left; width: 10%; border: 1px solid #eee; padding: 5px;">Is Switched?</th>
                <th style="text-align: left; width: 20%; border: 1px solid #eee; padding: 5px;">Pertinent Financial Info</th>
            </tr>';

            foreach($therapyChangeAndReco as $bs) {
                ...
                $mainDetails .= '
                    <tr>
                        ...
                        <td style="text-align: left; width: 20%; border: 1px solid #eee; padding: 5px;">'.$bs->medication_description.'</td>
                        <td style="text-align: left; width: 15%; border: 1px solid #eee; padding: 5px;">'.$bs->status_name.'</td>
                        <td style="text-align: left; width: 15%; border: 1px solid #eee; padding: 5px;">'.$bs->last_provider_that_sent_rx.'</td>
                        <td style="text-align: left; width: 10%; border: 1px solid #eee; padding: 5px;">'.$bs->is_switched.'</td>
        ...
        if(isset($user->id)) {
            ...
            $authEmployee = $this->therapyChangeAndReco[0]->user->employee;
            if(isset($authEmployee->id)) {
                $createdBy = $authEmployee->firstname.' '.$authEmployee->lastname;
            }
        }

        $subtext = 'TRP: Clinical > Therapy change+reco';   
        $button = '';
        $actionUrl = '';
