...
class AccountingAndFinanceRepository
{
    use FileIconUtil;
    ...
    )
    {
        ...
        $this->page_parent_id = 73;
        $this->pageIds = [74,75,76,77,82,83,84];
    ...
    public function getDataTable($request)
    {
        ...
        if(!empty($page_code)) {
            if(auth()->user()->can('accounting_and_finance.'.$page_code.'.delete'))
            {
                $hideD = '';
            }
        } else {
            if(auth()->user()->canany([
                'accounting_and_finance.proforma_and_budget.delete'
                , 'accounting_and_finance.weekly_financial_snapshots.delete'
                , 'accounting_and_finance.monthly_income_statement.delete'
                , 'accounting_and_finance.payroll_percentage.delete'
            ]))
            {
                $hideD = '';
            }
    ...
    public function store($request)
    {
        ...
        if ($request->file('files')) {
            ...
            foreach ($files as $key => $file) {
                $path = "/$this->env/admin/accounting-and-finance/$page_code/$folder_id/";
    ...
    public function delete($id)
    {
        ...
        if(isset($file->folder)) {
            ...
            $name = 'accounting_and_finance.'.$page->code.'.delete';
            ...
            if(auth()->user()->can($name)) {
                ...
                $transactionLog->function = 'AccountingAndFinanceRepository.delete';
