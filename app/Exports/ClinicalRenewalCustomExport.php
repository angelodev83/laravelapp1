...
class ClinicalRenewalCustomExport extends BaseExport implements FromCollection, WithHeadings, WithMapping, WithEvents, ShouldAutoSize, WithStyles
{
    private $filterArray = [];
    ...
    */
    ...
    {
        $query = ClinicalRenewal::with(['patient', 'status'])
            ->select('patient_id', 'rx_number', 'renew_date', 'call_attempts', 'status_id', 'telebridge', 'reason_for_denial', 'created_at');
    ...
    public function map($row): array
    {
        ...
        $renew_date = !empty($row->renew_date) ? Carbon::parse($row->renew_date)->format('m/d/Y') : '';

        $date_today = $this->getCurrentPSTDate('Y-m-d');
        $date1 = Carbon::createFromFormat('Y-m-d', $row->renew_date);
