...
class ClinicalRxDailyCensusCustomExport extends BaseExport implements FromCollection, WithHeadings, WithMapping, WithEvents, ShouldAutoSize, WithStyles
{
    private $filterArray = [];
    ...
    */
    ...
    {
        $query = ClinicalRxDailyCensus::with(['patient']);
        ...
        if(isset($this->filterArray['search'])) {
            ...
            if(!empty($term)) {
                ...
                $query = $query->where(function($q) use($term, $patientIdArr){
                    ...
                    $q->orWhere('birth_date', 'like', $term);
                    $q->orWhere('scripts_received', 'like', $term);
