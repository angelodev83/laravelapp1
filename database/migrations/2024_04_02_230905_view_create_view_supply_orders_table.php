...
return new class extends Migration
{
    ...
    {
        DB::statement(" 
            CREATE OR REPLACE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `view_supply_orders` AS (
                SELECT
                    ...
                    `supply_order_items`.`actual_quantity`        AS `actual_quantity`,
                    `supply_orders`.`file_id`                     AS `file_id`,
                    `files`.`filename`                            AS `file_name`,
                    `files`.`path`                                AS `file_path`,
                    `files`.`mime_type`                           AS `file_mimetype`
                FROM `supply_orders`
                LEFT JOIN `store_statuses`
                    ON `supply_orders`.`shipment_status_id` = `store_statuses`.`id`
                LEFT JOIN `files`
                    ON `files`.`id` = `supply_orders`.`file_id`
