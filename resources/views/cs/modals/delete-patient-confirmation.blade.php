      ...
      <div class="modal-body">
            <form name="delete_form"  id="delete_form"  >
                  ...
                  <ul>
                      ...
                      <li>Any associated prescriptions</li>
                  </ul>
           </form>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
        <button type="button" class="btn btn-danger" id="delete_btn" onclick="DeletePatient()">DELETE</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal modal-md" id="multiple_delete_form_modal" tabindex="-1" style="display:none;">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
...
<script>
  function ShowConfirmDeleteForm(firstname,lastname,id) {
        $('#delete_form_modal').modal('show');
        $('#delete_form #patient_id').val(id);
        $('#patient_name').html('Patient: '+firstname+' '+lastname);
  }
  ...
  function DeletePatient(id) {
                ...
                data.patient_id = $('#delete_form #patient_id').val();
             
                $.ajax({
                      headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                      type: "POST",
                      url: "/admin/patients/delete_patient_via_ajax",
                      data: JSON.stringify(data),
                      contentType: "application/json; charset=utf-8",
                      dataType: "json",
                      success: function(msg) {
                        $("#delete_btn").val('DELETE').removeAttr('disabled');
                         $('#row-'+data.patient_id +'').remove();
