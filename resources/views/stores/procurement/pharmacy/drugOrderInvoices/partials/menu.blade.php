    ...
    <div class="card-body" id="ir-monthly-control-counts-fm-menu" >
        ...
        <div class="fm-menu">
            <div class="list-group list-group-flush"> 
                ...
                <a href="/store/procurement/{{request()->id}}/pharmacy/drug-order-invoices/{{request()->year}}/0" class="list-group-item py-2 {{request()->month_number == 0 ? 'ir-monthly-control-counts-folder-card-selected fw-bold' : ''}}"><i class='fa-regular fa-folder me-2'></i>All Files</a>

                @for($i = 1; $i <= 12; $i++)
                    @if(isset(request()->month_number))
                        @if(request()->month_number == $i)
                            <a href="/store/procurement/{{request()->id}}/pharmacy/drug-order-invoices/{{request()->year}}/{{$i}}" class="selected-month operations-submenu-month ir-monthly-control-counts-folder-card-selected list-group-item py-2" id="menu_folder_{{$i}}">
                        ...
                        @else
                            <a href="/store/procurement/{{request()->id}}/pharmacy/drug-order-invoices/{{request()->year}}/{{$i}}" class="list-group-item operations-submenu-month py-2" id="menu_folder_{{$i}}">
