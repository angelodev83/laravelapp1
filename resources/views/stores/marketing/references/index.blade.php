...
<!--start page wrapper -->
		<div class="page-wrapper">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-12 col-lg-9">
                                ...
                                @include('stores/marketing/references/partials/folders')
                            </div>
                            <div class="col-12 col-lg-3">
                                @canany($permissions['create'])
                                @if(!empty($page_id))
                                <div class="mb-3 d-grid w-100">
                                    <button class="btn btn-default d-flex align-items-center justify-content-center" style="background-color: #5e17eb; color: white;" onclick="clickUploadBtn()">
                                        <i class="p-2 fa fa-cloud-arrow-up me-2"></i>Upload Documents
                                    </button>
                                </div>
                                @endif
                                @endcanany
                                @include('stores/marketing/references/partials/recent-upload-files')
                            </div>
                        </div>
                        <div class="row ms-2 me-3">
                            <div class="col-12 col-lg-12">
                                @include('stores/marketing/references/partials/table')
                            </div>
                        </div>
            ...
            @include('stores/marketing/references/modals/delete')
            @include('stores/marketing/references/modals/add')
            @include('stores/marketing/references/modals/edit-folder')
...
<script>
    ...
    function loadDocuments() {
        ...
        const dt_table = $('#dt_table').DataTable({
            ...
            destroy: true,
            ajax: {
                url: "/store/marketing/references/data",
                type: "POST",
                headers: {
                ...
                data: function (data) {
                    ...
                    // data.parent_page_id = 85;
                    data.page_id = page_id;
                    data.page_code = 'references';
                    if(folder_id) {
            ...
            columns: [
                ...
                { data: 'custom_data', name: 'custom_data', title: 'Score', orderable: false, searchable: false,
                    render: function(data, type, row) {
                        let custom_data = data ? JSON.parse(data) : {};
                        let score = custom_data.score ? custom_data.score*1 : 0;
                        let stars = '';
