...
<!--start page wrapper -->
    <div class="page-wrapper">
        <div class="page-content">
            ...
            <div class="row">
                <div class="col-xl-4 col-md-12">
                    ...
                    @include('stores/clinical/brandSwitchings/partials/summary')
                </div>
                    ...
                    <div class="card">
                        <div class="card-header dt-card-header">
                            ...
                            <div class="row mx-2">
                                ...
                                <div class="col-9">
                                    @can('menu_store.clinical.brand_switchings.create')
                                    ...
                                    @can('menu_store.clinical.brand_switchings.delete')
        ...
        @include('stores/clinical/brandSwitchings/modal/add')
        @include('stores/clinical/brandSwitchings/modal/edit')
        @include('stores/clinical/brandSwitchings/modal/delete')
        @include('stores/clinical/brandSwitchings/modal/delete-all')
...
<script>
    ...
    let selectedDate = '';
    let $_CALENDAR_URL = '/store/clinical/'+{{request()->id}}+'/brand-switchings/date-with';
    ...
    $(document).ready(function() {
        ...
        const dt_table = $('#table').DataTable({
            scrollX: true,
            serverSide: true,
            stateSave: false,
            processing: true,
            ...
            searching: true,
            ajax: {
                url: `/store/clinical/brand-switchings/data`,
                type: "GET",
                headers: {
            ...
            columns: [
                ...
                { data: 'patient_name', name: 'patient_name', title: 'Patient Name', render: function(data, type, row) {
                    return `${row.formatted_patient_name}`;
                } },
                { data: 'branded_medication_description', name: 'branded_medication_description', title: 'Brand Reco'},
                { data: 'is_switched', name: 'is_switched', title: 'Is Switched', render: function(data, type, row) {
                    if(row.is_switched != '' && row.is_switched != null) { 
                        let bg_class = row.is_switched == 'Yes' ? 'success' : 'danger';
                        return `<span class="badge bg-${bg_class} px-3 me-3">${row.is_switched}</span>`;
                    } else {
                        return '';
                    }
                } },
                { data: 'pertinent_financial_info', name: 'pertinent_financial_info', title: 'Pertinent Financial Info'},
                { data: 'total_paid_claims', name: 'total_paid_claims', title: 'Total Paid Claims', className: 'text-end'},
                { data: 'cost', name: 'cost', title: 'Cost', className: 'text-end'},
                { data: 'id', name: 'id', title: 'Profit', className: 'text-end', orderable: false, searchable: false, render: function(data, type, row) {
                    let profit = row.total_paid_claims - row.cost;
                    return `<b>${profit.toFixed(2)}</b>`;
                } },
    ...
    function updateCheckboxesState() {
        let currentPageCheckboxes = $('input[type="checkbox"].row-checkbox', table.rows({ 'page': 'current' }).nodes());
        ...
        $('input[type="checkbox"].row-checkbox', table.rows().nodes()).each(function() {
            let id = $(this).val();
    ...
    function clickExportBtn() {
        ...
        let url = `brand-switchings/export`;
    ...
    function saveImportSingleExcel()
    {
        ...
        proceedImportSingleExcel('/store/clinical/'+menu_store_id+'/brand-switchings/import', data);
    }
    ...
    function generateSummary()
    {
        ...
        $.ajax({
            ...
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            type: "POST",
            url: `/store/clinical/brand-switchings/summary`,
    ...
    function clickSendMailBtn()
    {
        ...
        $.ajax({
            ...
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            type: "POST",
            url: `/store/clinical/brand-switchings/send-mail`,
            data: JSON.stringify(data),
            contentType: "application/json; charset=utf-8",
            dataType: 'json',
            success: function(response) {
    ...
    function customizeImportModal()
    {
        ...
        $('#import_single_excel_modal #guide_table').html(`
            <p class="my-1 py-0">
                ** Check file <i>Clinical Import Excel Template - Brand switching IOU.xlsx</i> from <a href="/store/knowledge-base/1/process-documents?folder_id=90" target="_blank">Knowledge Base > Process Documents > <u>Templates</u></a>
            </p>
            <label class="form-label">** Please follow this Excel File Format (Sample ONLY Template)</label>
            <p class="my-1 py-0">
                <small>The <b class="text-primary">Date</b> will based on the 1st column from excel first. If no date inputted, it will based on the <u>calendar</u> selected from the form.</small>
            </p>
                ...
                <table class="table table-border" style="width:100%;">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>Generic Medication Name 1</td>
                            <td>Branded Medication Name A</td>
                        ...
                        <tr>
                            <td>8/16/2024 (optional)</td>
                            <td>137381</td>
                            <td>Doe, John Test</td>
                            <td>Generic Medication Name 2</td>
                            <td>Branded Medication Name B</td>
