...
<div id="renewal-container" class="gap-3 d-flex container-lists">
    @foreach($renewalStatus as $stat)
        <div id="card-renewal-status-{{$stat['id']}}" class="card rounded-4 card-renewal-status shadow-none" 
        ...
        ">
            ...
            <div class="p-1">
                ...
                <div id="renewal-status-{{$stat['id']}}" class="px-2 mb-2 content draggable-lists" 
...
<script>
    ...
    function loadBoardData(showLoading = false)
    {
        ...
        let hide_archive_class = 'd-none';
        let hide_delete_class = 'd-none';
        let hide_all_class = 'd-none';

        @can('menu_store.clinical.renewals.archive')
            hide_archive_class = '';
            hide_all_class = '';
        @endcan

        @can('menu_store.clinical.renewals.delete')
            hide_delete_class = '';
            hide_all_class = '';
        @endcan
        ...
        $.ajax({
            ...
            type: "POST",
            url: `renewals/filter-board-data`,
            ...
            success: function(res) {
                ...
                for (let g in groups) {
                    ...
                    for(let d in details) {
                        ...
                        let appendHtml = `
                            <div id="renewal-card-data-id-${raw.id}" class="hover-card card m-0 p-0 mb-2" data-renewal-id="${raw.id}" data-renewal-status-id="${raw.status_id}">
                                <div class="card-body m-0 p-2" onclick="openRenewalCard(${raw.id}, ${raw.is_archived})">
                                ...
                                <div class="dropdown dots ${hide_all_class}">
                                    <button type="button" class="btn dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                                        <i class="fas fa-ellipsis-v"></i>
                                    </button>
                                    <ul class="dropdown-menu">
                                        <li class="${hide_archive_class}">${archiveHtml}</li>
                                        <li class="${hide_delete_class}"><a class="dropdown-item fw-medium d-flex text-danger" href="javascript:clickDeleteBtn(${raw.id}, '${raw.rx_number}');"> Delete <i class="fa fa-trash-can ms-auto text-end text-danger"></i></a></li>
                                    </ul>
                                </div>
                            </div>
                        `;
                        ...
                        $(`#renewal-status-${g}`).append(appendHtml);

                        let h = $(`#renewal-card-data-id-${raw.id}`).height();
                        h+=8;
                    ...
                    // if(compute_card_height < $_responsive_scroll_height) { // TODO: un-comment if it is still scrollable via monitor height
                        ...
                        if(compute_card_height == 0) {
                            $(`#card-renewal-status-${g}`).height(120);
                        } else {
                            let computed = compute_card_height;
                            if(g == 924) {
                                computed+=18;
                            }
                            $(`#card-renewal-status-${g}`).height(computed+80);
                        }
                    // } // TODO: un-comment if it is still scrollable via monitor height

                    if(flag === false) {
                        $(`#renewal-status-${g}`).append('No Records Found.');
                    }
 
                    if(scrollable === true) {
                        $(`#card-renewal-status-${g}`).height($_responsive_card_height);
                        $(`#renewal-status-${g}`).height($_responsive_scroll_height);
                        new PerfectScrollbar(`#renewal-status-${group.id}`);
                    }
                }
        ...
        $('.draggable-lists').sortable({
            ...
            update: function(event, ui) {
                if (!isAjaxRequestInProgress) {
                    ...
                    const renewal_id = ui.item.data('renewal-id');
                    let old_status_id = ui.item.data('renewal-status-id')+'';
                    
                    let new_status_id = $(ui.item[0]).closest('.draggable-lists').attr('id');
                    new_status_id = new_status_id.slice(15);

                    console.log('dragging', this.id, renewal_id);

                    console.log("old status", old_status_id);
                    console.log("new status", new_status_id);

                    // if(old_status_id != new_status_id) {
                        updateDetails('status_id', renewal_id, new_status_id);
                    // }
    ...
    function clickUnarchiveBtn(renewal_id, rx_number)
    {
        updateDetails('is_archived', renewal_id, 0);
    }
</script>