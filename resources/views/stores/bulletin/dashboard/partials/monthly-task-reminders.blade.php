            ...
            <div class="d-flex align-items-center">
                <div>
                    <h6 class="mb-1 text-secondary">
                        <i class="fa-regular fa-calendar-days fa-sm me-2"></i>Monthly Task Reminders
                    </h6>
                    <small class="px-4">{{count($bulletinMonthlyTaskRecent)}} Most Recent(s)</small>
                </div>
                <div class="ms-auto">
                    <a href="/store/bulletin/{{request()->id}}/task-reminders">
                        <button class="btn btn-sm btn-outline-secondary">View More</button>
                    </a>
                </div>
            </div>
        </div>

        <div id="bulletin-monthly-tasks-recent-dashboard" class="customers-list pt-0 p-3 mb-3">
            @if(count($bulletinMonthlyTaskRecent) > 0)
                @foreach ($bulletinMonthlyTaskRecent as $item)
