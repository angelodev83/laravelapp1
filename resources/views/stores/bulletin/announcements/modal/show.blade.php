...
<script>
    function showStoreAnnouncementModal(id){
        let btn = document.querySelector(`#bulletin-announcement-edit-btn-${id}`);
        ...
        tagAsReadBulletinAnnouncementV1(id)
        console.log("fire show modal")

        $('#show_announcement_modal').modal('show');
    }

    function tagAsReadBulletinAnnouncementV1(id)
    {
        console.log("fire tag as read")
        $.ajax({
