...
<!--start page wrapper -->
		<div class="page-wrapper">
            ...
            <div class="mt-4 card">
                        ...
                        <div class="d-flex">
                            ...
                            <div class="float-end ms-auto">
                                @if (isset($task->status->name))
                                    <h2><span class="badge badge-task bg-{{$task->status->color}} px-5 ms-auto">{{$task->status->name}}</span></h2>
                                @endif
                            </div>
                        </div>
                        <p class="mb-0">
                            Assigned to: <b>{{ isset($task->assignedTo->id) ? $task->assignedTo->firstname. ' ' .$task->assignedTo->lastname : 'NA' }}</b>
                        </p>
                        <hr class="mt-1">
                ...
                <div class="card-footer d-flex">
                    <small>Created by: <b>{{ $task->user->employee->getFullName() }}</b></small>
                    <small class="ms-auto">{{ date('M d, Y H:iA', $task->create_at) }}</small>
                </div>
            </div>
        ...
        @include('stores/bulletin/taskDocuments/modals/delete')
...
<script>
    ...
    function loadDocuments() {
        ...
        const dt_table = $('#dt_table').DataTable({
            ...
            destroy: true,
            ajax: {
                url: "/store/bulletin/task-documents/data",
                type: "POST",
                headers: {
            ...
            columns: [
                ...
                { data: 'tag_labels', name: 'tag_labels', title: 'Tags' , orderable: false, searchable: false},
    ...
    function showEditModal()
    {
        ...
        tinymce.get("edescription").setContent(task.description);

        $("input#esubject").val(task.subject);
        $("textarea#edescription").val(task.description);
        $("input#eid").val(task.id);
        $("input#efrom").val('view');

        $( '#eassigned_to_employee_id' ).select2( {
            theme: "bootstrap-5",
            width: $( this ).data( 'width' ) ? $( this ).data( 'width' ) : $( this ).hasClass( 'w-100' ) ? '100%' : 'style',
        ...
        var emp_name = task.assigned_to ? (task.assigned_to.firstname+' '+task.assigned_to.lastname) : '';

        $("#eassigned_to_employee_id").append("<option selected value='"+task.assigned_to_employee_id+"'>"+emp_name+"</option>");

        searchSelect2Api('eassigned_to_employee_id', 'edit_task_modal','/admin/search/user-employee');
        getStatusDropDown('estatus_id', task.status_id);
        ...
        $('#edit_task_modal #for-file').html(fileInput); 
        $('#edit_task_modal #show-documents').imageuploadify();
        
        $("#edit_task_modal .imageuploadify-container").remove();
        $('#edit_task_modal .imageuploadify-message').html('Drag&Drop Your File(s) Here To Upload');     
    ...
    function getStatusDropDown(_dropdown_id, _status_id) {
        const data = {category: 'task'};
        $.ajax({
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            type: "POST",
    ...
    $('#upload_documents').change(function () {
        if(this.files.length){
            ...
            formData.append("task_id", tix_id);
            ...
            $.ajax({
                ...
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type: "POST",
                url: `/store/bulletin/task-documents/add`,
                data: formData,
                contentType: false,
                processData: false,
                dataType: "json",
