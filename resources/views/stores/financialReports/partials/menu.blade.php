        ...
        <div class="fm-menu">
            <div class="list-group list-group-flush"> 
                @if(in_array(request()->page_id, $pageIds['all']))
                    <a id="financial-reports-lgi-all-files" href="/store/financial-reports/{{request()->id}}/documents/all/54" class="list-group-item py-2"><i class='fa-regular fa-folder me-2'></i>All Files</a>
                @endif

                @if(in_array(request()->page_id, $pageIds['all_eod_reports']))
                    <a id="financial-reports-lgi-all-files" href="/store/financial-reports/{{request()->id}}/documents/all/60" class="list-group-item py-2"><i class='fa-regular fa-folder me-2'></i>All Files</a>
                @endif

                @if(in_array(request()->page_id, $pageIds['all_transaction_receipts']))
                    <a id="financial-reports-lgi-all-files" href="/store/financial-reports/{{request()->id}}/documents/all/68" class="list-group-item py-2"><i class='fa-regular fa-folder me-2'></i>All Files</a>
                @endif

                @foreach($allPages as $p)
                    @canany(['menu_store.financial_reports.'.$p->code.'.index', 'menu_store.financial_reports.'.$p->code.'.create', 'menu_store.financial_reports.'.$p->code.'.update', 'menu_store.financial_reports.'.$p->code.'.delete'])
                        <a id="financial-reports-lgi-{{$p->code}}" 
                            href="/store/financial-reports/{{request()->id}}/documents/{{$p->id}}" 
