...
<!--start page wrapper -->
        ...
        @include('stores/compliance/audit/modals/delete')
@stop
...
<script>
    ...
    function loadDocuments() {
        ...
        const dt_table = $('#dt_table').DataTable({
            ...
            dom: 'fBtip',
            buttons: [
                @can('menu_store.cnr.audit.create')
                {
            ...
            destroy: true,
            ajax: {
                url: "/store/compliance/audit/data",
                type: "POST",
                headers: {
    ...
    $('#upload_documents').change(function () {
        if(this.files.length){
            ...
            formData.append("tag_code", 'pbm_audit');
            formData.append("tag_type", 'audit');

            $.ajax({
                //laravel requires this thing, it fetches it from the meta up in the head
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type: "POST",
                url: `/store/compliance/${menu_store_id}/audit/add`,
                data: formData,
                contentType: false,
                processData: false,
                dataType: "json",
