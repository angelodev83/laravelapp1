<script>
    "use strict";
    let dt = new Date();
    dt = new Date(dt.toLocaleString('en-US', { timeZone: 'America/Los_Angeles' }));
   
    function renderDate() {
        let dt2 = new Date();
        let dateString = new Date(dt2.toLocaleString('en-US', { timeZone: 'America/Los_Angeles' }));
        ...
        let today = new Date(dt2.toLocaleString('en-US', { timeZone: 'America/Los_Angeles' }));
    ...
    function prepareKpiContent(kpiData) {
        const listItems = kpiData.map(data => {
            let is_existing = data.is_existing;
            if(is_existing == 'false' || is_existing == false) {
                return `<li><span class='text-red'>${data.dFirstname} ${data.dLastname}</span></li>`;
            } else {
                return `<li>${data.dFirstname} ${data.dLastname}</li>`;
            }
        ...
        return new Promise((resolve, reject) => {
            ...
            success: function(data) {
                ...
                error: function(err) {
                    ...
                    console.error('Error fetching from '+$_CALENDAR_URL+' data', err);
                    reject(err);
                }
            });
        });
